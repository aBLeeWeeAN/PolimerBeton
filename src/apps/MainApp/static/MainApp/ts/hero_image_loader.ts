...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const interval = 50; // интервал опроса
    let waited = 0;

    const insertHeroImages = () => {
        if (window.heroImages) {
            const picture_html = `
                <picture>
            ...
            waited += interval;
            setTimeout(insertHeroImages, interval);
        } else {
            console.warn("heroImages не появились в течение 10 секунд.");
        }
    };
